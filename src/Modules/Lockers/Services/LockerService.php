...
final class LockerService
{
    ...
    {
        $sql = 'SELECT public_id AS id, clinic_id, name, location, is_active, created_at, updated_at
    ...
    public function get(string $clinicId, string $publicId): ?array
    {
        $stmt = $this->pdo->prepare(
            'SELECT public_id AS id, clinic_id, name, location, is_active, created_at, updated_at
    ...
    public function create(string $clinicId, CreateLockerDTO $dto): array
    {
        ...
        $stmt = $this->pdo->prepare(
            'INSERT INTO lockers (id, public_id, clinic_id, name, location, is_active, created_at, updated_at)
             VALUES (:id, :public_id, :clinic_id, :name, :location, :is_active, NOW(), NOW())
             RETURNING public_id AS id, clinic_id, name, location, is_active, created_at, updated_at'
        );
        $stmt->execute([
            'id' => $id,
            'public_id' => $publicId,
    ...
    public function patch(string $clinicId, string $publicId, PatchLockerDTO $dto): ?array
    {
        $currentStmt = $this->pdo->prepare(
            'SELECT name, location, is_active FROM lockers WHERE clinic_id = :clinic_id AND public_id = :public_id LIMIT 1'
        );
