...
namespace App\Modules\Species\Services;
...
final class SpeciesService
{
    ...
    {
        $sql = 'SELECT id, name, slug, is_active, created_at, updated_at FROM species WHERE 1=1';
        $params = [];
        if ($active !== null) {
    ...
    public function get(string $speciesId): ?array
    {
        $stmt = $this->pdo->prepare(
            'SELECT id, name, slug, is_active, created_at, updated_at
             FROM species WHERE id::text = :id LIMIT 1'
        );
    ...
    public function create(CreateSpeciesDTO $dto, AuditActor $actor): array
    {
        $id = Uuid::v4()->toRfc4122();
        $slug = Slug::from($dto->name);
        $stmt = $this->pdo->prepare(
            'INSERT INTO species (id, name, slug, is_active, created_at, updated_at)
        ...
        $this->audit->recordAdd('species', $presented['id'], $actor->userId, $actor->clinicId, $presented);

        return $presented;
    }

    public function patch(string $speciesId, PatchSpeciesDTO $dto, AuditActor $actor): ?array
    {
        $before = $this->get($speciesId);
        if ($before === null) {
            return null;
        }
        ...
        $this->audit->recordEdit('species', $speciesId, $actor->userId, $actor->clinicId, $before, $after);

        return $after;
    }

    public function softDelete(string $speciesId, AuditActor $actor): bool
    {
        $stmt = $this->pdo->prepare(
            'UPDATE species SET is_active = FALSE, updated_at = NOW() WHERE id::text = :id'
        );
        $stmt->execute(['id' => $speciesId]);

        if ($stmt->rowCount() > 0) {
            $this->audit->recordDelete('species', $speciesId, $actor->userId, $actor->clinicId);
        }
