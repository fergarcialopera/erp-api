...
final class IncidentsEndpointTest extends BaseApiTestCase
{
    ...
    {
        $payload = ['title' => 'Inc', 'description' => 'Desc', 'severity' => 'HIGH'];
    ...
    public function testCreateIncidentValidation(): void
    {
        $res = $this->request(
            'POST',
            '/api/v1/incidents',
            ['title' => '', 'description' => '', 'severity' => 'WRONG'],
    ...
    public function testIncidentsAreIsolatedByClinic(): void
    {
        ...
        $this->request(
            'POST',
            '/api/v1/incidents',
            ['title' => $title, 'description' => 'Tenant A only', 'severity' => 'LOW'],
            $this->authHeaderFor('admin@clinic.local')
        );

        $listA = $this->request('GET', '/api/v1/incidents', null, $this->authHeaderFor('staff@clinic.local'));
        $listB = $this->request('GET', '/api/v1/incidents', null, $this->authHeaderFor('staff2@clinic.local'));
